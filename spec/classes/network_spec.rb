  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      describe 'Change Defaults' do
        ...
        context 'primary' do
          before(:each) { params.merge!(primary: 'eth1') }
          it { is_expected.to compile }
          it do
            is_expected.to contain_host('network.example.com').with(
              ip: '192.0.2.3',
        ...
        context 'purge_hosts' do
          before(:each) { params.merge!(purge_hosts: false) }
          it { is_expected.to compile }
      ...
      describe 'check bad type' do
        ...
        context 'dummy4' do
          before(:each) { params.merge!(dummy4: true) }
          it { is_expected.to raise_error(Puppet::Error) }
        end
        context 'dummy6' do
          before(:each) { params.merge!(dummy6: true) }
          it { is_expected.to raise_error(Puppet::Error) }
        end
        context 'sysctl' do
          before(:each) { params.merge!(sysctl: true) }
          it { is_expected.to raise_error(Puppet::Error) }
        end
        context 'additional_hosts' do
          before(:each) { params.merge!(additional_hosts: true) }
          it { is_expected.to raise_error(Puppet::Error) }
        end
        context 'primary' do
          before(:each) { params.merge!(primary: true) }
          it { is_expected.to raise_error(Puppet::Error) }
        end
        context 'prefer_ipv4' do
          before(:each) { params.merge!(prefer_ipv4: 'foobar') }
          it { is_expected.to raise_error(Puppet::Error) }
        end
        context 'purge_hosts' do
          before(:each) { params.merge!(purge_hosts: 'foobar') }
          it { is_expected.to raise_error(Puppet::Error) }
        end
      end
    end
